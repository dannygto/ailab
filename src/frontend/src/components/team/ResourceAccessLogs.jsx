 ...
 * 资源访问日志组件
 */
const ResourceAccessLogs = ({ resourceId, resourceType }) => {
  const { getResourceAccessLogs } = useTeamService();
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      // 调用API获取日志
      const response = await getResourceAccessLogs(resourceId, params);
  ...
  useEffect(() => {
    loadLogs();
  }, [resourceId, page, pageSize, filters]);
  ...
  const renderFilterDialog = () => (
    <Dialog
      ...
      fullWidth
    >
      <DialogTitle>过滤访问日志</DialogTitle>
      <DialogContent>
...
export default ResourceAccessLogs;
