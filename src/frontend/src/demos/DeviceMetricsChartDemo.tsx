...
const DeviceMetricsChartDemo: React.FC = () => {
  ...
  const devices: Device[] = [
    ...
    },
    {
      ...
      lastSeen: '2025-06-10T09:30:15Z',
      capabilities: ['远程观测', '图像捕获', '视频录制', '测量'],
    ...
    },
    {
      ...
      description: '多通道数据记录器，适用于长期数据采集',
      ...
      lastSeen: '2025-06-15T11:40:30Z',
      capabilities: ['多通道记录', '本地存储', '自动校准'],
      supportedProtocols: ['FTP', 'SFTP', 'HTTP'],
      dataFormats: ['CSV', 'Excel', 'SQLite'],
      configuration: { 
        channels: 16,
        sampleRate: '1s',
        storageMode: 'circular'
      },
      metadata: { 
  ...
  const generateDataPoints = (deviceId: string, count: number): DeviceDataPoint[] => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const point: DeviceDataPoint = {
        ...
        sensorType: 'multiple',
        value: 50 + Math.random() * 100, // 默认传感器值
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={8}>
          <DeviceMetricsChart
            ...
            onRefreshIcon={updateDataPoints}
            height={600}
