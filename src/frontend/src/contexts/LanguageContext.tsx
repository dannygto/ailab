...
// 支持的语言
export type Language = 'zh-CN' | 'en-US' | 'ug-CN' | 'mn-CN' | 'mn-MN' | 'ii-CN' | 'yi-CN' | 'bo-CN';
...
const translationData: Record<Language, Record<string, string>> = {
  ...
  'ug-CN': { 'app.name': 'AILAB پلاتفورماسى' },
  'mn-CN': { 'app.name': 'AILAB Платформ' },
  'mn-MN': { 'app.name': 'AILAB Платформ' },
  'ii-CN': { 'app.name': 'AILAB ꅉꀱ' },
  'yi-CN': { 'app.name': 'AILAB ꅉꀱ' },
  'bo-CN': { 'app.name': 'བཟོ་རིག་རིག་ནུས་ཚོད་ལྟའི་སྡིངས་ཆ།' }
};
...
export const LanguageSwitcher: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  ...
  const languages = [
    ...
    { code: 'ug-CN', name: 'ئۇيغۇرچە' },
    { code: 'mn-CN', name: 'ᠮᠣᠩᠭᠣᠯ ᠬᠡᠯᠡ' },
    { code: 'mn-MN', name: 'Монгол хэл' },
    { code: 'ii-CN', name: 'ꆈꌠꉙ' },
    { code: 'yi-CN', name: 'ꆈꌠꁱꂷ' },
    { code: 'bo-CN', name: 'བོད་སྐད།' }
  ];

  // 简洁模式下只显示当前语言和切换选项
  if (compact) {
    const currentLang = languages.find(lang => lang.code === language) || languages[0];
    const nextLang = language === 'zh-CN' ? languages[1] : languages[0];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {languages.map(lang => (
        <button
          ...
          style={{
            background: language === lang.code ? '#f0f0f0' : 'none',
            border: '1px solid #ddd',
            cursor: 'pointer',
            padding: '8px',
            borderRadius: '4px',
            color: language === lang.code ? '#333' : '#666',
            fontSize: '14px',
            textAlign: 'left'
          }}
        >
