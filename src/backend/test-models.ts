...
import { aiService, AI_MODEL_CONFIGS } from './src/services/ai.service';

// 模拟配置用于测试
const testConfigs: { [modelId: string]: any } = {
  // OpenAI测试配置
...
async function testModelConfigurations() {
  ...
  const totalModels = Object.keys(AI_MODEL_CONFIGS).length;
  ...
  for (const [modelId, modelConfig] of Object.entries(AI_MODEL_CONFIGS)) {
    console.log(`\n📋 测试模型: ${modelId}`);
...
export { testModelConfigurations };
