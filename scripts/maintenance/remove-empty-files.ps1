...
# 初始化日志
"删除空文件操作日志 - 开始于 $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File -FilePath $logFile -Encoding utf8# 寻找空文件（大小为0字节的文件）
Write-Log "开始扫描空文件..."
...
if ($emptyFiles.Count -eq 0) {
    ...
    "# 空文件报告" | Out-File -FilePath $reportFile
    "生成日期: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')`n" | Out-File -Append -FilePath $reportFile
    "## 发现的空文件" | Out-File -Append -FilePath $reportFile
    ...
    foreach ($file in $emptyFiles) {
        ...
        # 添加到报告
        "- `"$relativePath`" (最后修改: $($file.LastWriteTime.ToString('yyyy-MM-dd')), 文件年龄: ${fileAge}天)" | Out-File -Append -FilePath $reportFile
        ...
        if ($fileAge -gt 90) {
            try {
                ...
                "  - ✓ 已删除 (超过90天未修改)" | Out-File -Append -FilePath $reportFile
            }
            catch {
                Write-Log "删除失败: $relativePath - $($_.Exception.Message)"
                "  - ❌ 删除失败: $($_.Exception.Message)" | Out-File -Append -FilePath $reportFile
            }
        }
        else {
            "  - ⚠️ 未删除 (修改时间在90天内)" | Out-File -Append -FilePath $reportFile
        }
    }
