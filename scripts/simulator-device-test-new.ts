...
async function testBasicConnection(deviceName: string, config: any): Promise<void> {
  ...
  const adapter = new SimpleTCPAdapter({
    host: 'localhost',
    port: config.port,
    delimiter: config.commandDelimiter || '\r\n',
  ...
  try {
    ...
    // 测试识别指令
    try {
      const identResponse = await adapter.sendCommand('*IDN?');
...
async function runTests(): Promise<void> {
  ...
  for (const device of simulatedDevices) {
    ...
    // 运行基础连接测试
    await testBasicConnection(device.name, {
      port: device.port,
      commandDelimiter: '\r\n',
    ...
    // 运行设备特定命令测试
    await testDeviceSpecificCommands(device.name, {
      port: device.port,
      commandDelimiter: '\r\n',
      connectionTimeout: 5000
    });
