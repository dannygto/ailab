...
import { DeviceDiscoveryService, DeviceDiscoveryEvents, DeviceType } from '../src/backend/device/adapters/tcp-socket/device-discovery';
 ...
 * 默认设备配置
 */
const DEFAULT_CONFIGS: { [key: string]: TCPSocketConnectionOptions & { deviceType: string, commandDelimiter: string, responseTimeout: number } } = {
  'GX-5000': {
    host: '192.168.1.100',
    port: 4001,
...
// 读取自定义配置文件
function loadCustomConfig(deviceName: string): TCPSocketConnectionOptions | null {
...
// 设置测试设备
function setupTestDevice(deviceName: string): TestDevice | null {
  ...
  const config: TCPSocketConnectionOptions = {
    ...
    ...(params.port ? { port: parseInt(params.port, 10) } : {})
  };
...
function createClient(device: TestDevice): boolean {
  try {
    ...
    // 设置事件监听
    device.client.on(TCPSocketEvents.CONNECTED.toString(), (data: any) => {
      console.log(`已连接到设备 ${device.name}: ${data.connectionId}`);
    });

    device.client.on(TCPSocketEvents.DISCONNECTED.toString(), (data: any) => {
      console.log(`断开与设备 ${device.name} 的连接: ${data.connectionId}`);
    });

    device.client.on(TCPSocketEvents.ERROR.toString(), (data: any) => {
      console.error(`设备 ${device.name} 连接错误:`, data.error.message);
    });

    device.client.on(TCPSocketEvents.DATA_RECEIVED.toString(), (data: any) => {
...
// 执行设备命令
async function executeCommand(device: TestDevice, command: string, description: string): Promise<TestResult> {
  if (!device.client) {
    return {
      device: device.name,
      command,
  ...
  try {
    // 准备命令字符串
    const delimiter = device.config['commandDelimiter'] || '\r\n';
    ...
    const responsePromise = new Promise<string>((resolve, reject) => {
      ...
      }, device.config['responseTimeout'] || 3000);

      // 设置一次性数据接收处理
      device.client?.once(TCPSocketEvents.DATA_RECEIVED.toString(), (data: any) => {
...
// 运行设备测试
async function runDeviceTest(device: TestDevice): Promise<boolean> {
  ...
  for (const cmd of device.commands) {
    const result = await executeCommand(device, cmd.command, cmd.description);
...
// 保存测试结果
function saveTestResults(device: TestDevice): void {
  ...
  const resultsFile = path.join(resultsDir, `${device.name}-${timestamp}.json`);
...
// 交互式命令行界面
function startInteractiveConsole(device: TestDevice): void {
  if (!device.client) {
  ...
  const promptUser = () => {
    rl.question('> ', async (input) => {
      ...
      if (!isNaN(cmdIndex) && cmdIndex > 0 && cmdIndex <= device.commands.length) {
        ...
        await executeCommand(device, cmd.command, cmd.description);
      } else {
        // 直接执行输入的命令
        await executeCommand(device, input, '用户输入命令');
      }
...
async function main(): Promise<void> {
  ...
  // 解析设备名称
  const deviceName = params.device || 'GX-5000'; // 默认使用GX-5000

  // 设置测试设备
  const device = setupTestDevice(deviceName);
  if (!device) {
  ...
  // 判断运行模式
  const mode = params.mode || 'test'; // 默认为自动测试模式

  if (mode === 'test') {
    // 自动测试模式
    const testPassed = await runDeviceTest(device);
    saveTestResults(device);
    ...
    startInteractiveConsole(device);
  } else {
    console.error(`未知模式: ${mode}，支持的模式: test, interactive`);
    process.exit(1);
  }
}
