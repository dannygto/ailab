...
while (-not $success -and $retryCount -lt $maxRetries) {
    try {
        ...
        if ($portCheck) {
            Write-Host "警告：端口3002仍被占用，尝试再次释放..." -ForegroundColor Yellow
            Kill-Process-On-Port -Port 3002
            Start-Sleep -Seconds 2
        }
        
        Write-Host "正在启动后端服务 (尝试 $($retryCount + 1)/$maxRetries)..." -ForegroundColor Cyan
        Write-Host "后端将在 http://localhost:3002 启动" -ForegroundColor Green
        Write-Host "请稍候，启动可能需要一些时间..." -ForegroundColor Yellow
        
        # 将启动命令写入临时脚本
        $tempScriptPath = "$env:TEMP\run-backend-$([Guid]::NewGuid().ToString()).ps1"
        @"
Set-Location "$($PSScriptRoot)/../backend"
