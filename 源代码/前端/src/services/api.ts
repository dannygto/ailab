...
class apiService {
      ...
      switch (status) {
        ...
        case 404:
          ...
          if (error.config?.url?.includes('health') || 
            ...
            console.warn(`?? api�˵� ${error.config.url} �����ڣ�ʹ�ý���ģʽ`);
            return;
          }
          toast.error('�������Դ������');
          break;
  ...
  async getdevices(params?: GetdevicesParams): Promise<Device[]> {
    ...
    return devices.map(device => ({
      ...
      isActive: (device as any).isActive ?? true,
      connectionStatus: 'connected' as const // ʹ�ü��ݵ��ַ���ֵ
    }));
  }
  ...
  async createDeviceReservation(deviceId: string, params: CreateDeviceReservationParams): Promise<DeviceReservation> {
    return this.deviceservice.createReservation(deviceId, params);
  }
