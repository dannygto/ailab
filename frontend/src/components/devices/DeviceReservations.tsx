...
import { AddIcon as AddIcon, DeleteIcon as DeleteIcon, WarningIcon as WarningIcon, AccountCircleIcon as AccountCircleIcon, InfoIcon as InfoIcon } from '../../utils/icons';
...
const DeviceReservations: React.FC<DeviceReservationsProps> = ({ 
  reservations, 
  devices,
  ...
  const [endTime, setEndTime] = useState<Date | null>(
    new Date(new Date().getTime() + 60 * 60 * 1000) // Default to 1 hour later
  );
  const [purpose, setPurpose] = useState('');
  const [title, setTitleIcon] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [timeConflict, setTimeConflict] = useState(false);  const [filteredReservations, setFilteredReservations] = useState<DeviceReservation[]>(reservations);
  ...
  // 渲染筛选器
  const renderFilters = () => (
    <div sx={{ mb: 2, display: 'flex', gap: 2 }}>
                  ...
                  <TableRow key={reservation.id}>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        label={
                          reservation.status === 'approved' ? '已批准' :
                          reservation.status === 'pending' ? '待审批' :
                          reservation.status === 'rejected' ? '已拒绝' :
                          reservation.status === 'CancelIconled' ? '已取消' :
