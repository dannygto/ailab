...
const DeviceMonitorDashboard: React.FC = () => {
  ...
  const { 
    ...
    try {
      ...
      // 为每个设备获取监控数据并转换为DeviceMetrics格式
      const metricsPromises = devices.map(async (device: any): Promise<DeviceMetrics> => {
        try {
          const monitoringData = await api.getDeviceMonitoringData(device.id);
          return {
            deviceId: device.id,
            deviceName: device.name,
            cpuUsage: monitoringData.metrics?.cpuUsage || Math.random() * 100,
            memoryUsage: monitoringData.metrics?.memoryUsage || Math.random() * 100,
            temperature: monitoringData.metrics?.temperature || Math.random() * 30 + 20,
            status: monitoringData.status || 'online'
          };
        } catch (error) {
          return {
            deviceId: device.id,
